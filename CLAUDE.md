...
- **A Meta no le va un dato personal.** `buildMetaEventParams()` (`src/lib/meta-pixel.ts`) es la única puerta hacia el píxel y solo deja pasar `tenant`, `location` y `content_category`; hay un test que fija la lista exacta de claves. Las pantallas del mesero **no se miden** (`isMeasuredPath()`), y el aviso al cliente sale del MISMO predicado que el script: no puede haber una página que mida sin avisar.
