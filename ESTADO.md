...
| Base de datos de producción | ✅ **Aplicadas hasta la `00056`** (dueño, 2026-09-08: `00047`, `00050`, `00051`, `00053`, `00054` y `00056`, todas). El esquema ya alcanza al código de `main`. La 00030 NUNCA aplicada (a propósito). La 00015 NO se aplica (reabre fuga). Huecos: `00048`, `00049`, `00052`, `00055`. **`00062` aplicada** (dueño, 2026-09-11, antes del push). **`00065` aplicada** (dueño, 2026-09-11, antes del push). **Escritas sin aplicar: `00058`, `00059`, `00061`, `00064`, `00066`** (la 00066: interruptor de domicilios para el AIOS, riesgo nulo, va ANTES del AIOS v1.12.0) |
...
0.DOMI **Planeta Wings ve «Los domicilios por WhatsApp no están activados en tu marca» y el AIOS ya tiene el
   botón (12).** En orden: (1) aplicar la **`00066`** en el Supabase del producto (dos funciones, riesgo nulo);
   (2) pushear el AIOS (`6323ced`, v1.12.0; Vercel lo despliega solo); (3) en el AIOS: Planeta Wings → sede →
   tarjeta Sistema → «Domicilios por WhatsApp» → **Encender**, cuando la línea ya reciba y el celular del
   operador esté en Domicilios → Autorizados de su panel. La bandera solo informa: ningún webhook la lee.
   Aparte: **el Supabase del AIOS devuelve `Gateway Timeout`** en `generate_due_payments` y en las lecturas de
...
0.SUSHI **Sushi Service: la difusión por la línea de coexistencia (Zernio), lo normal por Twilio
   ...
   (`712ff58`, `origin/main` = `05b7eb8`):** el Golden Bullet lee `admin_settings.golden_bullet_provider` y con `zernio` + cuenta
...
0.PLANTILLAS **Rediseño del sistema de plantillas: el dueño decidió el 12 (una por una desde el panel, urgente). El prompt para la sesión que lo construye está en `docs/prompts/2026-09-12-plantillas-una-por-una.md`.** **Hecho y pusheado el 12 (`8cf5ed4` = `origin/main`): el editor deja escribir el «Nombre en WhatsApp»** (`suggestedName`, `normalizeTemplateName()`, 409 si ya se usó, y el 400 de Zernio ya dice el motivo). Con eso Planeta Wings se crea de a una con nombre nuevo (`bienvenida_pw`…) sin tocar las 13 pegadas. Sigue pendiente el punto 1 del prompt (adoptar lo que ya está en la WABA) y los puntos 2 y 3. Antes:
