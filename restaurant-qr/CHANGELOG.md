...
## [0.22.0] — 2026-05-02 — Production-Ready: Rebrand + Security + QR + Campaign ROI

### REBRAND
- **Sushi Service → Constelarys Fidelity System**: todos los strings de UI centralizados en `src/lib/branding.ts`
- `package.json` renombrado a `constelarys-fidelity` v0.22.0
- Branding configurable vía env vars (`NEXT_PUBLIC_BRAND_NAME`, etc.) para clone-per-client sin tocar código

### SECURITY (7 fixes)
- **ADDED** `src/lib/rate-limit.ts` — rate limiter in-memory (sliding window) para MVP
- **FIXED** `/api/check-in`: rate limiting por IP (30 lookups/min, 5 registros/hora, 20 check-ins/min)
- **FIXED** `/api/webhook/delivery`: fail-closed si `WEBHOOK_DELIVERY_SECRET` no configurado + rate limit 60/min
- **FIXED** `/api/health/twilio`: ahora requiere autenticación admin
- **FIXED** `validateCronSecret()`: eliminado dev fallback que retornaba true
- **ADDED** Security headers en `next.config.ts`: HSTS, X-Frame-Options, X-Content-Type-Options, Referrer-Policy, Permissions-Policy, CSP
- **AUDITED** No hay secrets hardcoded en ningún archivo fuente

### QR ENHANCEMENTS
- **ADDED** Color picker para QR (persistido en localStorage)
- **ADDED** Upload de logo PNG transparente (superpuesto al centro del QR con ECC-H)
- **CHANGED** QR ahora se genera como PNG completo con: nombre del negocio (header) + logo + número de mesa (footer prominente) — listo para imprimir sin editar
- Archivo descargado nombrado dinámicamente: `{brand-slug}-qr-mesa-{n}.png`

### CAMPAIGN ROI + ATTRIBUTION
- **ADDED** `/api/dashboard/campaigns/efficiency` — endpoint que calcula por campaña:
  - total_sent, attributed_visits (ventana 7 días), conversion_rate, estimated_revenue
  - Usa amounts reales de visitas si existen, avg_ticket como fallback
- **ADDED** `CampaignEfficiencyChart` — gráfica en dashboard con:
  - KPI cards (conversión global, revenue atribuido, campañas ejecutadas)
  - Barras coloreadas por tipo (manual/birthday/reactivation)
  - Línea de conversión %
  - Selector de período (30/90/180/365 días)

### BUGS FIXED
- **FIXED** Check-in: Google Contacts sync no se ejecutaba en path de recompensa (solo en path normal)
- **FIXED** Check-in: si no hay `reward_template_sid` pero hay recompensa, ahora hace fallback a `welcome_back` template
- **FIXED** Manual campaigns: envío secuencial (timeout en Vercel con >50 clientes) → ahora en batches paralelos de 10
- **FIXED** Manual campaigns: N+1 en `getNextReward()` → pre-computa hints por grupo de visitas
- **FIXED** Manual campaigns: bulk update de `last_campaign_at` en 1 query (antes N queries)
- **FIXED** Birthday cron: no filtraba por `accepts_marketing=true` — violación de opt-out corregida

### DOCS
- **ADDED** `docs/DEPLOYMENT_GUIDE.md` — guía completa clone-per-client + costos + checklist
