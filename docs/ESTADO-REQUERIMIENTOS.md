...
|---|------|--------|-----------|---|
| 3 | Personalización del QR | ✅ **HECHO** (rama `feat/tarjeta-visual`) | La config del QR Studio vive en `tenants.config.qr_studio` (migración `00048`, **sin aplicar**); el QR de `CustomerCard.tsx` lleva color de marca + logo con `level="H"`. Ver `docs/features/identidad-visual.md` | NO |
